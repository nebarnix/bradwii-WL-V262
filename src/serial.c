...
void evaluatecommand(char portnumber, unsigned char *data)
{
    ...
    if (command == MSP_IDENT) { // send rx data
        ...
        for (int x = 0; x < 4; ++x)
            sendandchecksumcharacter(portnumber, 0);    // 32 bit "capability"
...
void serialcheckportforaction(char portnumber)
{
    ...
    while ((numcharsavailable = lib_serial_numcharsavailable(portnumber)) != 0) {
        if (serialreceivestate[portnumber] == SERIALSTATEGOTCOMMAND) {
            ...
            int spaceneeded = 40;
            if (serialcommand[portnumber] == MSP_BOXNAMES)
            ...
            if (serialreceivestate[portnumber] == SERIALSTATEIDLE) {
                if (c == '$')
                    serialreceivestate[portnumber] = SERIALSTATEGOTDOLLARSIGN;
            } else if (serialreceivestate[portnumber] == SERIALSTATEGOTDOLLARSIGN) {
                if (c == 'M')
                    serialreceivestate[portnumber] = SERIALSTATEGOTM;
                else
                    serialreceivestate[portnumber] = SERIALSTATEIDLE;
            } else if (serialreceivestate[portnumber] == SERIALSTATEGOTM) {
                if (c == '<')
                    serialreceivestate[portnumber] = SERIALSTATEGOTLESSTHANSIGN;
                else
                    serialreceivestate[portnumber] = SERIALSTATEIDLE;
